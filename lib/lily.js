...
export function classifyTaskIntent(task) {
  ...
  if (CLARIFICATION_PATTERNS.some((pattern) => pattern.test(normalized))) {
    return { intent: "CLARIFICATION", mode: "ASK", action: "RESPOND" };
  }

  if (QUESTION_PATTERNS.some((pattern) => pattern.test(normalized))) {
    return { intent: "QUESTION", mode: "ASK", action: "RESPOND" };
  }

  if (CASUAL_PATTERNS.some((pattern) => pattern.test(normalized))) {
    return { intent: "CASUAL", mode: "ASK", action: "RESPOND" };
  }

  return { intent: "COMMAND", mode: "ASK", action: "RESPOND" };
}
...
Do not execute or claim to execute external tools.`
  ...
  try {
    ...
    return {
      task,
      summary: "Lily completed the request but could not structure the result.",
      notes: ["Please retry the request for a clean formatted response."]
    };
  }
}
