...
import { formatDuration } from "./task-service.js";
...
export async function processNextTask({
  ...
  try {
    ...
    const runningMetadata = {
      ...
      current_activity: "Understanding the objective"
    };
    ...
    await notify(
      task.chat_id,
      `Task ${task.id.slice(0, 8)} started. ETA: ${formatDuration(estimatedDurationSeconds)}.`
    );
    ...
    const result = await runner(task, {
      ...
      onProgress: async ({ progress, message }) => {
        ...
        await db.updateTask(task.id, {
          progress,
          metadata: {
            ...(current?.metadata || runningMetadata),
            current_activity: message
          }
        });
        const milestone = [75, 50, 25].find(
          (threshold) => progress >= threshold && lastProgressMilestone < threshold
        );
        if (milestone) {
          lastProgressMilestone = milestone;
          await notify(
            task.chat_id,
            `Task ${task.id.slice(0, 8)}: ${progress}% complete\n${message}`
          );
        }
      }
    });
